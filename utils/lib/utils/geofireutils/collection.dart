...
class GeoFireCollectionRef {
  ...
  Stream<List<DocumentSnapshot>> within({
    ...
    Iterable<Stream<List<DistanceDocSnapshot>>> queries = area.map((hash) {
      final tempQuery = _queryPoint(hash, field, status);
    ...
    var filtered = mergedObservable.map((List<DistanceDocSnapshot> list) {
      var mappedList = list.map((DistanceDocSnapshot distanceDocSnapshot) {
        ...
        var geoPointField = (distanceDocSnapshot.documentSnapshot.data() as Map<String, dynamic>)[fieldList[0]];
      ...
      final filteredList = strictMode
          ? mappedList
              .where((DistanceDocSnapshot doc) => doc.distance! <= radius * 1.02 // buffer for edge distances;
                  )
              .toList()
          : mappedList.toList();
  ...
  /// construct a query for the [geoHash] and [field] and [status]
  Query _queryPoint(String geoHash, String field, String status) {
    final end = '$geoHash~';
    final temp = _collectionReference;
    return temp!.orderBy('$field.geohash').startAt([geoHash]).endAt([end]).where("status", isEqualTo: status);
  }
